---
title: Turn your Starlight docs into AI-ready documentation
description: Let users instantly chat with ChatGPT, Claude, or any AI about your documentation.
template: splash
hero:
  title: ""
---

import {
  Hero,
  Features,
  Faq,
  Cta,
  CompaniesLogos,
} from "@dlcastillop/starlight-marketing-components";
import Copy from "@lucide/astro/icons/copy";
import MessageCircle from "@lucide/astro/icons/message-circle";
import Bot from "@lucide/astro/icons/bot";
import Joystick from "@lucide/astro/icons/joystick";
import MessageCircleX from "@lucide/astro/icons/message-circle-x";
import Share from "@lucide/astro/icons/share-2";
import LayoutGrid from "@lucide/astro/icons/layout-grid";

<Hero
  title={{
    fistPart: "Turn your Starlight docs into",
    highlightedPart: "AI-ready",
    secondPart: "documentation",
  }}
  description="Let users instantly chat with ChatGPT, Claude, or any AI about your documentation."
  primaryCta={{
    title: "Get Started",
    href: "docs/getting-started",
  }}
  secondaryCta={{
    title: "Become a Sponsor",
    href: "https://ko-fi.com/dlcastillop",
  }}
/>

<CompaniesLogos
  title="Trusted by developers at"
  images={[
    {
      whiteSrc: "/microsoft-white.avif",
      blackSrc: "/microsoft-black.avif",
      alt: "Microsoft logo",
    },
  ]}
/>

<Features
  highlightedTitle="Supercharge your documentation"
  title="Make your docs accessible to developers and AI"
  description="Turn your Starlight site into a knowledge source ready to share, query, and process."
  features={[
    {
      name: "Markdown Tools",
      description: "View and copy documentation in clean Markdown format.",
      icon: Copy,
    },
    {
      name: "Open in ChatGPT, Claude, or any AI",
      description:
        "Open documentation directly in ChatGPT, Claude, or any AI with the full page context. Let your users ask specific questions about your docs without manual copy-pasting.",
      icon: MessageCircle,
    },
    {
      name: "llms.txt Generator",
      description:
        "Automatically generate an optimized llms.txt file for your entire site. Follows community standards so LLMs can better index and understand your documentation.",
      icon: Bot,
    },
    {
      name: "Full Control Over Actions",
      description:
        "Choose which actions to display on your documentation. Show or hide default actions and add your own custom actions to fit your workflow and documentation needs.",
      icon: Joystick,
    },
    {
      name: "Page-Level Control",
      description:
        "Disable page actions on pages that aren't ideal for LLM consumption. Perfect for excluding changelog pages, legal documents, release notes, or any content that doesn't benefit from AI interaction.",
      icon: MessageCircleX,
    },
    {
      name: "One-Click Sharing",
      description:
        "Share any documentation page with a single click. Choose from multiple sharing options including social media, email, and direct links to help users spread your documentation effortlessly.",
      icon: Share,
    },
    {
      name: "Choose where to display the actions",
      description:
        "You can display the actions wherever you prefer: below the title or below the page's table of contents.",
      icon: LayoutGrid,
    },
  ]}
/>

<Faq
  faqs={[
    {
      question: "Does this plugin work with all Starlight themes?",
      answer:
        "Yes! The plugin is designed to work seamlessly with any Starlight theme. It integrates with Starlight's built-in components and styling system, so it will automatically match your site's look and feel.",
    },
    {
      question: "Can I customize which actions appear on my pages?",
      answer:
        "Yes! You have full control over which actions are displayed. You can show or hide any of the default actions and add your own custom actions to match your specific needs and workflow.",
    },
    {
      question: "How is the Markdown generated?",
      answer:
        "The plugin uses vite-plugin-static-copy to create copies of your .md and .mdx files. This exposes the original Markdown files, making them accessible and ready to be consumed by LLMs or copied by users while preserving all formatting, code blocks, and structure.",
    },
    {
      question: "Can I disable page actions on specific pages?",
      answer:
        "Yes! You can disable page actions on individual pages using frontmatter configuration. This is useful for pages that aren't ideal for LLM consumption, such as changelogs, legal documents, release notes, or any content that doesn't benefit from AI interaction.",
    },
  ]}
/>

<Cta
  title="Ready to make your docs AI-friendly?"
  description="Join developers who are making their documentation more accessible, shareable, and AI-ready with just one plugin."
  primaryCta={{
    title: "Get Started",
    href: "docs/getting-started",
  }}
  secondaryCta={{
    title: "Become a Sponsor",
    href: "https://ko-fi.com/dlcastillop",
  }}
/>
