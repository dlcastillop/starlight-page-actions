...
<Aside>
  All default actions are internationalized and support English, Spanish, Portuguese, French,
...
<Aside type="tip">
  The `href` should include the complete URL with the appropriate query parameter structure for the
...
<Aside>
  If all default actions are set to `false` and no custom actions are specified, the page actions
  section will not be displayed.
</Aside>

## Internationalization

Custom actions can be internationalized. See the [Internationalization guide](/docs/guides/internationalization) for more details.
